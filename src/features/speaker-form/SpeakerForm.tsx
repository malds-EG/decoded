...
function SuccessView({ onDone }: { onDone: () => void }) {
  ...
  return (
    <motion.div
      className="flex min-h-[400px] flex-col items-center justify-center gap-6 py-8 text-center"
    ...
    >
      ...
      <motion.button
        onClick={onDone}
        className="rounded-lg bg-red px-8 py-3 font-body font-semibold text-white transition-opacity hover:opacity-90"
  ...
  return (
    <div className="mx-auto max-w-5xl rounded-2xl border border-white/20 bg-black/70 p-8 backdrop-blur-md md:p-10">
      <AnimatePresence mode="wait">
        {submitted ? (
          <SuccessView key="success" onDone={() => onSuccess?.()} />
        ) : (
          <motion.div
            key="form"
            initial={{ opacity: 1 }}
